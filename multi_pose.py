...
import cv2
import os
from center_face_utils.image import flip, color_aug
from center_face_utils.image import get_affine_transform, affine_transform
from center_face_utils.image import gaussian_radius, draw_umich_gaussian, draw_msra_gaussian
from center_face_utils.image import draw_dense_reg
from center_face_utils.utils import Data_anchor_sample
from center_face_utils.Randaugmentations import Randaugment
import math
  ...
  def __getitem__(self, index):
    ...
    for k in range(num_objs):
      ...
      if (h > 0 and w > 0) or (rot != 0):
        ...
        ct = np.array(
          [(bbox[0] + bbox[2]) / 2, (bbox[1] + bbox[3]) / 2], dtype=np.float32)       # 人脸的中心坐标  Face center
        ct_int = ct.astype(np.int32)                        # 整数化    # Integerization
        ...
        reg_mask[k] = 1                                     # 是否需要用于计算误差   Whether it needs to be used to calculate the error
        # if w*h <= 20:
        #     wight_mask[k] = 15

        num_kpts = pts[:, 2].sum()                           # 没有关键点标注的时哦   When there are no key points
        if num_kpts == 0:                                    # 没有关键点标注的都是比较困难的样本   The samples without key points are more difficult samples
          # print('没有关键点标注')
        ...
        for j in range(num_joints):
          if pts[j, 2] > 0:
            ...
            if pts[j, 0] >= 0 and pts[j, 0] < output_res and \
              ...
              kps[k, j * 2: j * 2 + 2] = pts[j, :2] - ct_int                # 4. 关键点相对于人脸bbox的中心的偏差   The deviation of the key point from the center of the face bbox
              kps_mask[k, j * 2: j * 2 + 2] = 1
              pt_int = pts[j, :2].astype(np.int32)                          # 关键点整数化    Key point integerization
              hp_offset[k * num_joints + j] = pts[j, :2] - pt_int           # 关键点整数化的偏差  Key point integerization deviation
              hp_ind[k * num_joints + j] = pt_int[1] * output_res + pt_int[0]   # 索引  index
              hp_mask[k * num_joints + j] = 1                                   # 计算损失的mask    Calculate the mask of the loss
              ...
              if ann['bbox'][2]*ann['bbox'][3] <= 16.0:                   # 太小的人脸忽略    Faces too small to ignore
